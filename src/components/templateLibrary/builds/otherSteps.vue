<template>
  <section class="other-step-container">
    <el-drawer :modal-append-to-body="false" title="Dockerfile Template Preview" :visible.sync="showDockerfile" direction="rtl">
      <Codemirror
      ...
      >
        <div class="dashed-container">
          ...
          <el-form-item label="Dockerfile Source" prop="source">
            <el-select size="small" v-model="buildConfig.post_build.docker_build.source" placeholder="Please Choose" @change="$refs.dockerBuildRef.clearValidate()">
              <el-option label="Code Repository" value="local"></el-option>
              <el-option label="Template Library" value="template"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="buildConfig.post_build.docker_build.source === 'local'" label="Dockerfile The Absolute Path Of" prop="docker_file">
            <el-input v-model="buildConfig.post_build.docker_build.docker_file" size="small">
              <template slot="prepend">$WORKSPACE/</template>
            </el-input>
          </el-form-item>
          <el-form-item v-if="buildConfig.post_build.docker_build.source === 'template'" label="Template Selection" prop="template_id">
            <el-select
              style="width: 90%;"
              size="small"
              filterable
      ...
      >
        <div class="dashed-container">
          ...
          <el-form-item label="Object Storage" prop="object_storage_id">
            <el-select size="small" v-model="buildConfig.post_build.object_storage_upload.object_storage_id" placeholder="Please select Object Storage" @change="$refs.objectStorageRef.clearValidate()">
