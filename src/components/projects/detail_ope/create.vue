<template>
  <el-dialog :fullscreen="true" custom-class="create-project" :before-close="handleClose" :visible.sync="dialogVisible">
    <header class="project-contexts-modal__header">{{isEdit?'Modify project information':'New Project'}}</header>
    <section class="project-contexts-modal__content">
      ...
      >
        ...
        <el-form-item label="Item Primary Key" prop="product_name" class="label-icon">
          <span slot="label">
            Project ID
            <el-tooltip effect="dark" content="The project ID is the globally unique identifier for the project resource，Used for citation and update of all resources under the project，Automatically generated by default，Also supports manual specification，Cannot be changed after creation" placement="top">
        ...
        <el-form-item prop="desc" label="Description">
          <el-input type="textarea" :rows="1" placeholder="Please enter a description" v-model="projectForm.desc"></el-input>
        </el-form-item>
        <el-form-item v-if="!isEdit" label="Project Type">
          ...
          <div class="type-link">
            ...
            <el-link
              ...
              target="_blank"
              :underline="false"
            >List of supported infrastructures</el-link>
          </div>
        </el-form-item>
        ...
        <div v-show="showAdvanced || isEdit">
          <el-form-item label="Access Permission" prop="public">
            <el-select v-model="projectForm.public" popper-class="access-permission">
              <el-option label="Private" :value="false">
                ...
                <div class="desc">This item is invisible to normal users of the system by default，Project administrators can add corresponding permissions to specific users by adding members。</div>
              </el-option>
              <el-option label="Public" :value="true">
                <div class="title">Public</div>
                <div class="desc">Users have public permissions by default，Can view the workflow in the project、Surroundings、Serve、Construct、Resources For Testing。</div>
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="Project Administrator" v-if="!isEdit" prop="admins">
            <el-select v-model="projectForm.admins" filterable multiple remote :remote-method="remoteMethod" :loading="loading" placeholder="Please enter username to search for users">
              <el-option
                v-for="user in users"
                :key="user.uid"
          ...
          <el-form-item v-if="!isEdit" label="Specified Cluster" prop="cluster_ids">
            <el-select filterable multiple clearable v-model="projectForm.cluster_ids" placeholder="Select the cluster resources used by the project">
    ...
    <footer class="project-contexts-modal__footer">
      <el-button type="primary" @click="submitForm('projectForm')">{{isEdit?'Confirm The Changes':'Create Now'}}</el-button>
    </footer>
  </el-dialog>
</template>
<script>
...
const validateProductName = (rule, value, callback) => {
  if (typeof value === 'undefined' || value === '') {
    callback(new Error('Fill in the project primary key'))
  } else {
    if (!/^[a-z0-9-]+$/.test(value)) {
      callback(new Error('Item primary key only supports lowercase letters and numbers，Special characters only support underscores'))
    } else {
      callback()
    }
  }
        ...
        admins: [
          {
            type: 'array',
            required: true,
            message: 'Please select a project administrator',
            trigger: 'change'
          }
        ],
        cluster_ids: [
          {
            type: 'array',
            required: true,
            message: 'Please select a cluster resource',
            trigger: 'change'
          }
        ],
        enabled: [
          {
            type: 'boolean',
            required: true,
            message: 'Please select whether to enable the project or not',
            trigger: 'change'
          }
        ],
        public: [
          { required: true, message: 'Project permissions cannot be empty', trigger: 'blur' }
        ]
      },
      allCluster: [],
      showAdvanced: false,
      projectTypeList: [
        {
          ...
          firstDesc: 'Infrastructure Use Kubernetes',
          secondDesc: 'Use Kubernetes YAML Manage and deploy services',
        ...
        },
        {
          ...
          firstDesc: 'Infrastructure Use Kubernetes',
          secondDesc: 'Use Helm Chart Manage and deploy services',
          icon: 'iconfont iconhelmrepo'
        },
        {
          type: 'external',
          title: 'K8s Managed Project',
          firstDesc: 'Host An Existing Kubernetes Resources In The Cluster',
          secondDesc: 'Support for service image updates',
          icon: 'iconfont iconvery-trustee'
        },
        {
          type: 'host',
          title: 'Host Project',
          firstDesc: 'Infrastructure using hosts',
          secondDesc: 'Deploy the upgrade service using a custom script',
