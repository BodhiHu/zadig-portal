...
export default {
  ...
  methods: {
    ...
    allowDrop (draggingNode, dropNode, type) {
      if (dropNode.data.label.indexOf('Boot Sequence') === -1) {
        return type !== 'inner'
      } else {
        return type === 'inner'
      }
    ...
    allowDrag (draggingNode) {
      return draggingNode.data.label.indexOf('Boot Sequence') === -1
    },
    generateNodeData () {
      const nodeData = []
      this.services.forEach((service, index) => {
        nodeData.push({
          label: `Boot Sequence ${index}`,
      ...
      nodeData.push({
        label: `Boot Sequence ${nodeData.length}`,
        children: []
      })
    ...
    startDrag (node) {
      ...
      if (this.nodeData[nodeLength - 1].children.length) {
        this.nodeData.push({
          label: `Boot Sequence ${nodeLength}`,
          children: []
        })
      }
    },
