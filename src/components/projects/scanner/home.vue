  ...
  >
    <div v-if="scannerList.length > 0" class="tab-container">
      <div class="scanner-container">
        <el-input v-model="selectKey" placeholder="Search Code Scan" class="search-scanner" prefix-icon="el-icon-search" clearable></el-input>
      </div>
    ...
    <div v-if="scannerList.length === 0 && !loading" class="no-product">
      ...
      <p>There are currently no code scans to show，Please manually create a new code scan</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    removeCodeScanner ({ name, id }) {
      this.$confirm(`Sure You Want To Delete ${name} Is This Configuration？`, 'Confirm', {
