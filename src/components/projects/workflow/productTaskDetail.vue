<template>
  <div class="workflow-task-detail workflow-or-pipeline-task-detail">
    <el-dialog :visible.sync="artifactModalVisible" width="60%" title="File Export" class="downloadArtifact-dialog">
    ...
    <div class="common-parcel-block">
      <div class="workflow-basic-info">
        <div class="basic-left">
          ...
          <el-form class="secondary-form" label-width="100px" label-position="left">
            ...
            <el-form-item label="Creator">{{ taskDetail.task_creator }}</el-form-item>
            <el-form-item v-if="taskDetail.task_revoker" label="Canceller">{{ taskDetail.task_revoker }}</el-form-item>
            <el-form-item label="Surroundings">{{ workflow.namespace }}</el-form-item>
            <el-form-item label="Duration">
              {{ taskDetail.interval }}
              <el-tooltip v-if="taskDetail.intervalSec<0" content="There may be inconsistencies between the local system time and the server，Please Sync。" placement="top">
                <i class="el-icon-warning" style="color: red;"></i>
              </el-tooltip>
            </el-form-item>
            <el-form-item v-if="taskDetail.releases && taskDetail.releases.length > 0 && taskDetail.status==='passed'" label="Delivery List">
              <router-link
        ...
        <div class="basic-right" v-if="buildSummary.length > 0 || jenkinsSummary.length > 0">
          <div class="primary-title not-first-child">Build Information</div>
          <div class="build-summary" v-if="buildSummary.length > 0">
            <el-table :data="buildSummary" style="width: 90%;" class="blank-background-header">
              <el-table-column label="Serve" min-width="160">
              ...
              <el-table-column label="Issue Track" width="160">
                <template slot-scope="scope">
                  <div v-if="scope.row.issues.length > 0">
                    ...
                    >
                      <p>Title: {{issue.summary?issue.summary:'*'}}</p>
                  ...
                  <span v-else>No Issue</span>
                </template>
              </el-table-column>
              <el-table-column label="Variable" width="100">
                <template slot-scope="{ row }">
                  <el-popover placement="left" width="400" trigger="hover">
                    ...
                    <el-button slot="reference" type="text">Check</el-button>
                  </el-popover>
                </template>
              </el-table-column>
            </el-table>
        ...
        <div class="basic-right version-info" v-if="taskDetail.workflow_args && taskDetail.workflow_args.version_args">
          <div class="primary-title not-first-child">Version Information</div>
          <el-form class="secondary-form" label-width="100px" label-position="left">
            <el-form-item label="Version Name">{{taskDetail.workflow_args.version_args.version}}</el-form-item>
            <el-form-item label="Version Description">{{taskDetail.workflow_args.version_args.desc}}</el-form-item>
            <el-form-item label="Version Label">
      ...
      <template v-if="buildDeployArray.length > 0">
        <div class="primary-title not-first-child">Environment Update</div>
        <el-alert
          class="description"
          v-if="jenkinsSummary.length > 0"
          show-icon
          title="Use Zadig Build Will Save You20%Build Time，It is recommended that you migrate your build process to Zadig"
          :closable="false"
          type="warning"
        ></el-alert>
        <div></div>
        ...
        >
          ...
          <el-table-column prop="_target" label="Serve" min-width="200px">
            <template slot-scope="scope">
              <span>{{$utils.showServiceName(scope.row._target,scope.row.buildv2SubTask.service)}}</span>
            </template>
          </el-table-column>

          <el-table-column label="Construct" min-width="250px">
            <template slot-scope="scope">
              <span :class="scope.row.buildOverallColor">{{ scope.row.buildOverallStatusZh }}</span>
              {{ scope.row.buildOverallTimeZh }}
              <el-tooltip v-if="scope.row.buildOverallTimeZhSec<0" content="There may be inconsistencies between the local system time and the server，Please Sync。" placement="top">
                <i class="el-icon-warning" style="color: red;"></i>
              </el-tooltip>
            </template>
          </el-table-column>
              ...
              <div v-if="scope.row.deploySubTasks">
                <template>
                  <span v-for="(task,index) in scope.row.deploySubTasks" :key="index">
                    ...
                    <el-tooltip v-if="calcElapsedTimeNum(task)<0" content="There may be inconsistencies between the local system time and the server，Please Sync。" placement="top">
                      <i class="el-icon-warning" style="color: red;"></i>
                    </el-tooltip>
                  </span>
                </template>
      ...
      <template v-if="artifactDeployArray.length > 0">
        ...
        >
          ...
          <el-table-column min-width="200px">
            <template slot="header">Deliverables Deployment</template>
            <template slot-scope="scope">
              <span :class="scope.row.buildOverallColor">{{ scope.row.buildOverallStatusZh }}</span>
              {{ scope.row.buildOverallTimeZh }}
              <el-tooltip v-if="scope.row.buildOverallTimeZhSec<0" content="There may be inconsistencies between the local system time and the server，Please Sync。" placement="top">
      ...
      <div v-if="testArray.length > 0" class="primary-title not-first-child">Product Testing</div>
      <template v-if="testArray.length > 0">
        <span class="secondary-title">Automated Test</span>
        <el-table
          :data="testArray"
          row-key="_target"
        ...
        >
          ...
          <el-table-column prop="_target" label="Name" width="200px">
            <template slot-scope="scope">
              <span>{{$utils.showServiceName(scope.row._target)}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="Operating Status">
            <template slot-scope="scope">
              ...
              <el-tooltip v-if="calcElapsedTimeNum(scope.row.testingv2SubTask)<0" content="There may be inconsistencies between the local system time and the server，Please Sync。" placement="top">
                <i class="el-icon-warning" style="color: red;"></i>
              </el-tooltip>
            </template>
          </el-table-column>
          ...
          <el-table-column label="Testing Report">
            <template slot-scope="scope">
              <span v-if="scope.row.testingv2SubTask.report_ready === true" class="show-test-result">
                <router-link :to="getTestReport(scope.row.testingv2SubTask, scope.row._target)">Check</router-link>
              </span>
            </template>
          </el-table-column>
      ...
      <template v-if="distributeArray.length > 0">
        <div class="primary-title not-first-child">Distribution Deployment</div>
        ...
        >
          ...
          <el-table-column label="Distribution" min-width="250px">
            <template slot-scope="scope">
              <template  v-if="scope.row.release_imageSubTask">
                <span :class="colorTranslation(scope.row.release_imageSubTask.distribute_info[0].distribute_status, 'pipeline', 'task')">{{'Mirror：' + myTranslate(scope.row.release_imageSubTask.distribute_info[0].distribute_status) }}</span>
              </template>
              <template v-if="scope.row.distribute2kodoSubTask">
                <span :class="colorTranslation(scope.row.distribute2kodoSubTask.status, 'pipeline', 'task')">{{ 'Object Storage：' + myTranslate(scope.row.distribute2kodoSubTask.status) }}</span>
              </template>
            </template>
          </el-table-column>

          <el-table-column label="Deploy" min-width="250px">
            <template slot-scope="scope">
...
export default {
  data () {
    return {
      ...
      rules: {
        version: [
          { required: true, message: 'Please fill in the version name', trigger: 'blur' }
        ]
      },
      argColumns: [
        {
